...
Kylin.TreeSelector = function(item,data,rootId,defaultValue){ 
    this._data = data; 
    this._item = item; 
    this._rootId = rootId; 
    if(defaultValue)this.defaultValue = defaultValue;
} 
Kylin.TreeSelector.prototype.createTree = function(){ 
...
Kylin.TreeSelector.prototype.createSubOption = function(len,current,next){ 
    var blank = ".."; 
    ...
    Kylin.TreeSelector.prototype.getLevel = function(datasources,topId,currentitem){ 
...
// 只能輸入數字，且第一數字不能為0
Kylin.digitalOnly = function(obj) {
 	// 先把非数字的都替换掉
 ...
 * 获取版本
 */
Kylin.getKylinMAllVersion = function(url){
	$.post(url,{},function(data,textStatus){
	   ...
	   if(json){
		   if(json.version && json.version!='same'){
			   $('.Kylinmall-version-tips').show();
			   $('#Kylinmall_version').html(json.version);
			   $('#Kylinmall_down').attr('href',json.downloadUrl);
		   }
 ...
 * 取文档内容实际高度  
 *******************/  
 Kylin.getScrollHeight = function()  
 {  
     return Math.max(document.body.scrollHeight,document.documentElement.scrollHeight);  
 }

 //只能輸入數字
 Kylin.isNumberKey = function(evt){
 ...
 //只能輸入數字和小數點
 Kylin.isNumberdoteKey = function(evt){
 ...
 //只能輸入數字和字母
 Kylin.isNumberCharKey = function(evt){
 	var e = evt || window.event; 
 	var srcElement = e.srcElement || e.target;	
 	var charCode = (evt.which) ? evt.which : event.keyCode;
...
Kylin.isChinese = function(obj,isReplace){
 	var pattern = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/i
...
//比较时间差
Kylin.getTimeDiff = function(startTime,endTime,diffType){
 ...
 * @returns
 */
Kylin.checkMinLength = function(o,minLength){
	if(Kylin.getBytes(o)<=minLength){
		return false;
	}
	return true;
}
...
<input type="hidden" id="logoImg" value="{$data['logoImg']}">
<img src="__ROOT__/{$data['logoImg']}" id="logoImg_preview" alt="">

Kylin.uploadAjax('uploadImg',Think.U('Admin/Index/uploadPic'),function(addr){
 $('#logoImg').val(addr);
  $('#logoImg_preview').attr('src','__ROOT__/'+addr);
});
 *
 *
 * 
 */
Kylin.uploadAjax = function (target,url,fun) {
    console.log(url);
    if(window.FormData){
 ...
 * @returns
 */
Kylin.checkMaxLength = function(o,maxLength){
	if(Kylin.getBytes(o)>maxLength){
		return false;
	}
	return true;
}
Kylin.msg = function(msg, options, func){
	var opts = {};
	//有抖動的效果,第二位是函數
...
Kylin.toJson = function(str,notLimit){
	var json = {};
	try{
...
//把Kylin-panel-full样式的表单设置布满屏幕高度
$(function () {
	if($('.Kylin-panel-full').height()<Kylin.pageHeight())$('.Kylin-panel-full').height(Kylin.pageHeight()-3);
});
 ...
 * 去除url中指定的参数(用于分页)
 */
Kylin.splitURL = function(spchar){
 ...
 * 替换url
 */
Kylin.replaceURL = function(url,ar){
...
Kylin.checkChks = function(obj,cobj){
	$(cobj).each(function(){
		$(this)[0].checked = obj.checked;
	})
}
Kylin.getChks = function(obj){
	var ids = [];
	$(obj).each(function(){
